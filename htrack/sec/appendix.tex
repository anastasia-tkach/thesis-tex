...
%Note that $\parssub = \proj_\posespace^T\parsHtrack$ is an orthogonal projection only when $\omegapcareg = 0$. 
...
\subsubsection*{Perspective projection Jacobian} 
...
%\MB{This is confusing. Are you linearizing w.r.t.\ a 3D point $\pointHtrack$ or the kinematic parameters $\parsHtrack$? And what is the linearization of a joint?}
% 
% 
%  
% 
...
\end{align*}
% 
% Joint bounds constraints are only added when $\parsHtrack_i$ is lower (higher) than the acceptable lower (higher) bound:
...
% \begin{eqnarray}
% \omegabounds \left[ \delta\parsHtrack_i - (\overline{\parsHtrack}_i-\parsHtrack_i-\epsilon)\right] = 0
% \\
% \omegabounds \left[ \delta\parsHtrack_i - (\underline{\parsHtrack}_i-\parsHtrack_i+\epsilon)\right] = 0
% \end{eqnarray}
% %
% 
%  
...
% 
%  
\subsubsection*{Data-driven (PCA)} 
The data-driven projection energy can be rewritten as
% To take into account of the PCA space in our optimization it is not sufficient to simply compute the projection $\tilde\parsHtrack$ and then optimize for its projection in an alternating fashion. Instead we first compute $\tilde\parsHtrack=\mathbf{P}^t\parsHtrack$, and then jointly optimize for $\hat\parsHtrack = [\parsHtrack, \tilde\parsHtrack]$ the concatenation of unknowns in full and latent space, resulting in the linear equations:
% The energy to optimize for can then be rewritten as $\|\mathbf{I}, -\mathbf{P}\hat\parsHtrack\|_2^2$, whose linearization produces the constraints:
% \begin{equation*}
% \bar{E}_{\text{pose}}  = \omegapcaproj \|\begin{bmatrix}\mathbf{I} & -\proj_\posespace\mathbf{M}\proj_\posespace^T \end{bmatrix}^T(\delta{\parsHtrack} + \parsHtrack - \boldsymbol{\mu}) \|_2^2.
% \end{equation*}
\begin{equation*}
...
%
\vspace{-.3in}
\subsection{Approximation using Linearized $\ell_2$ Distance.}
...
% \begin{equation}
% d = \|\pointHtrack - \proj_{\handmodel}(\pointHtrack,\parsHtrack)\|_2
% \label{eq:jacobian3d}
% \end{equation}
% \begin{equation}
% \mathbf{n} = \frac{\pointHtrack - \proj_{\handmodel}(\pointHtrack,\parsHtrack)}{\|\pointHtrack - \proj_{\handmodel}(\pointHtrack,\parsHtrack)\|_2}
% \label{eq:jacobian3d}
% \end{equation}
%
% 
...
% 
%  
\subsubsection*{Silhouette alignment} The silhouette energy is expressed in screen space, and therefore employs the perspective projection Jacobian $\Jproj(\pointHtrack)$, where $\pointHtrack$ is the 3D location of a rendered silhouette point $\pixelHtrack$. Similarly to the point cloud alignment the linearization can be expressed as
...
% where,
% \begin{equation}
% d = \|\pixelHtrack - \proj_{\SilhoSensor}(\pixelHtrack,\parsHtrack)\|_2,
% \label{eq:jacobian3d}
% \end{equation}
% and
% \begin{equation}
% \mathbf{n} = \frac{\pixelHtrack - \proj_{\SilhoSensor}(\pixelHtrack,\parsHtrack)}{\|\pixelHtrack - \proj_{\SilhoSensor}(\pixelHtrack,\parsHtrack)\|_2}.
% \label{eq:jacobian3d}
% \end{equation}
% 
% 
...
% 
%
\input{htrack/fig/collision.tex}
% \vspace{-1cm}
\subsubsection*{Collision} \Figure{collision} illustrates the necessary notation with a 2D example, where $\pointHtrack_i$ and $\pointHtrack_j$ are the end-points of the shortest segment between the two cylinders axes. The linearized energy is defined as
% 
\begin{equation*}
...
\end{equation*}
%
where $\mathbf{n}_i$ is the surface normal at $\pointHtrack_i$ (as shown in \Figure{collision}), and $\mathbf{d} = (\pointHtrack_i - \pointHtrack_j)$. 
%  
% 
%  
...
\min_{\delta\parsHtrack} \: \bar{E}_{\text{3D}} + \bar{E}_{\text{2D}} + \bar{E}_{\text{wrist}} + \bar{E}_{\text{pose}} + \bar{E}_{\text{kin.}} + \bar{E}_{\text{temp.}} + \bar{E}_{\text{damp}},
\end{equation*}
%
and update our hand pose using the update $\parsHtrack = \parsHtrack+\delta\parsHtrack$. \new{Note that since our energies are written in the form:
% 
\begin{equation*}
\Sigma_i \bar{E}_i = \Sigma_i \|\jacobian_i \delta\parsHtrack -\residuals_i\|_2^2,
\end{equation*}}
% 
...
\label{eq:solve}
\end{equation}
To stabilize the optimization, we introduce a damping energy $\bar{E}_{\text{damp}} = \lambda\|\delta\parsHtrack\|_2^2$, where $\lambda = 100$.

\vspace{-.15in}
\new{
\subsection{CPU/GPU Optimization}
\label{app:gpu}
...
Basic notation:
\begin{itemize}
    \item $\pixelHtrack$ is a pixel!!
    \item $\pointHtrack$ is a 3D point
    \item $\parsHtrack \in R^{\color{red}27}$ hand parameter space, joint angles + global rigid
    \item $\dpars \in R^{\color{red}27}$ computed differential update
    \item $\PointsSensorHtrack, \PointsRender$: resp sensor and rendered 3D points
	...
	\item $\DataSensor=\{\PointsSensorHtrack, \SilhoSensor\}$ current data frame
	\item $\history=\{\parsHtrack_{t-1}, \parsHtrack_{t-2}, ...\}$ tracking history
